...
def record_unify_fail(df):
    df=df[df['整车厂企业简称_G']=='']
    df[['企业名称_G']].to_csv('table_evpromot_unify_fail.csv')
...
def main():
    df=pd.read_csv('table_announce_ev.csv',encoding='gbk').astype(str)
    df = df.replace(to_replace=re.compile(r'.*nan.*'), value='')
    names=[]
    column_max=['总质量(kg)_G','整备质量(kg)_G','整车长_G','整车宽_G',
                '整车高_G','轴距(mm)_G','最高车速(km/h)_G']
    df=get_data_max(df, column_max)
    for i in range(0,len(df)):
        names.append(get_company_name_unify(df.loc[i,'企业名称_G']))
    df['整车厂企业简称_G']=names
    df.set_index('企业名称_G').to_csv('table_change1.csv')
